...
do 
  CURRENT_CONTRIBUTION_COUNT=$(echo $contribution | jq -r ".contributionCount")
  CONTRIBUTION_DATE=$(echo $contribution | jq -r ".date")
  CONTRIBUTION_YEAR=${CONTRIBUTION_DATE:0:4}
  YEARS_CONTRIBUTION[CONTRIBUTION_YEAR]=$(( YEARS_CONTRIBUTION[CONTRIBUTION_YEAR] + CURRENT_CONTRIBUTION_COUNT ))
  if [[ $CURRENT_CONTRIBUTION_COUNT -gt 0 ]]; then
    if [[ $CURRENT_CONTRIBUTION_COUNT -gt $MAX_CONTRIBUTION ]]; then
      MAX_CONTRIBUTION=$CURRENT_CONTRIBUTION_COUNT
    fi 
    STREAK_COUNT=$(( STREAK_COUNT + 1 ))
    if [[ $STREAK_COUNT -eq 1 ]]; then
      CURRENT_STREAK_DATE=$(cat "${USERNAME}/contributions/${USERNAME}.json" | jq -r '.['$(( INDEX - 1 ))'].date')
    fi
    ...
    CONTRIBUTION_COUNT=$(( CONTRIBUTION_COUNT + CURRENT_CONTRIBUTION_COUNT ))
  else
